...
if(is_admin()) {
	...
	add_filter('widget_update_callback', 'gk_widget_rules_update', 10, 4);
	add_action('sidebar_admin_setup', 'gk_widget_rules_add_control'); 
	add_action('admin_enqueue_scripts', 'gk_widget_rules_load_scripts');
	
	function gk_widget_rules_load_scripts($hook) {
		if($hook != 'widgets.php') {
			return;
		}
	...
	function gk_widget_rules_update($instance, $new_instance, $old_instance, $widget) {	
	...
	function gk_widget_rules_control() {	
		...
		gk_widget_rules_control_styles_list($params[0]['widget_id'], $id, $responsive_mode, $users_mode, $style_css);
	}
	
	function gk_widget_rules_add_control() {	
		global $tpl;
		...
		foreach ( $wp_registered_widgets as $id => $widget ) {	
			...
			$wp_registered_widget_controls[$id]['callback'] = 'gk_widget_rules_control';		
		}
	}
	
	function gk_widget_rules_control_styles_list($widget_name, $id, $responsive, $users, $css = '') {
		echo '<div>';
		//
...
if(!is_admin() && !class_exists('GK_Widget_Rules_Front_End')) {
	class GK_Widget_Rules_Front_End {
		 ...
		 *
		 **/
		static function gk_widget_rules_condition($mode, $input, $users) {
			// Example input:
		...
		static function gk_widget_rules_filter_widgets($sidebars_widgets) {	
			$config = gk_widget_rules_get_config();
			...
			foreach($sidebars_widgets as $sidebar => $widgets) {	
				...
				foreach($widgets as $index => $id) {	
					...
					if(!isset(self::$conditions[$id])) {
						self::$conditions[$id] = self::gk_widget_rules_condition($type, $rules, $users);
					}
		...
		static function gk_widget_rules_add_classes($params) {
			$config = gk_widget_rules_get_config();
	...
	add_filter('sidebars_widgets', array('GK_Widget_Rules_Front_End', 'gk_widget_rules_filter_widgets'), 10);
	add_filter('dynamic_sidebar_params', array('GK_Widget_Rules_Front_End', 'gk_widget_rules_add_classes'), 10);
}
